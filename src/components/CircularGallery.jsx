...
class App {
  ...
  update() {
    ...
    if (this.medias && this.medias[0]) {
      ...
      if (itemWidth > 0) {
        const rawIndex = Math.round(this.scroll.current / itemWidth);
        const length = this.medias.length / 2;
        let activeIndex = rawIndex % length;
        if (activeIndex < 0) activeIndex += length;
        activeIndex = ((activeIndex % length) + length) % length;
        
        if (this.onActiveIndexChange && activeIndex !== this.lastActiveIndex) {
